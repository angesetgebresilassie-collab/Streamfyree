...
class MusicViewModel(app: Application) : AndroidViewModel(app) {
    ...
    private fun resolveAndPlay(track: Track, native: Boolean) {
        viewModelScope.launch(Dispatchers.IO) {
            runCatching {
                ...
                track.copy(id = if (native) r.id else track.id, title = r.title, artist = r.artist,
                    youtubeUrl = r.youtubeUrl, streamUrl = r.streamUrl, durationMs = r.durationMs, lyricVideo = true)
            }.onSuccess { resolved ->
                ...
                val url = resolved.streamUrl ?: return@onSuccess
                val metadata = MediaMetadata.Builder().setTitle(resolved.title).setArtist(resolved.artist)
                    .setAlbumTitle(resolved.album).setArtworkUri(resolved.artwork?.let(Uri::parse)).build()
                val c = controller ?: runCatching { controllerFuture.get() }.getOrNull()
                c?.setMediaItem(MediaItem.Builder().setMediaId(resolved.id).setUri(url).setMediaMetadata(metadata).build())
                c?.prepare()
                c?.play()
            }.onFailure {
                _state.value = _state.value.copy(error = it.message ?: "Playback resolution failed")
            }
        }
    }

    fun enqueue(track: Track) {
        if (_queue.value.none { it.id == track.id }) { _queue.value += track; persistQueue(_queue.value) }
    }
    fun removeFromQueue(track: Track) { _queue.value = _queue.value.filterNot { it.id == track.id }; persistQueue(_queue.value) }
    ...
    fun refreshProgress() { controller?.let { _progress.value = PlaybackProgress(it.currentPosition, it.duration.coerceAtLeast(0)) } }
    ...
    fun unsaveTrack(track: Track) {
        ...
        _library.value = LibraryState(saved); persistLibrary(saved)
    }
    ...
    private fun decodeList(raw: String?): List<Track> = runCatching {
        ...
        val a = JSONArray(raw); List(a.length()) { decodeTrack(a.getString(it)) }.filterNotNull()
    ...
    private fun decodeTrack(raw: String): Track? = runCatching {
        val o=JSONObject(raw); Track(o.getString("id"),o.getString("title"),o.getString("artist"),
            o.optString("album"),o.optString("artwork").ifBlank{null},o.optLong("duration"),
